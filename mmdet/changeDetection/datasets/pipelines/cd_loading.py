...
class LoadCdImageFromFile(object):
    ...
    def __call__(self, results):
        ...
        leftImgName = filename
        rightImgName = filename.replace("left","right")

        preImg = mmcv.imread(leftImgName)
        curImg = mmcv.imread(rightImgName)
        #img = mmcv.imread(filename)
        if self.to_float32:
            preImg = preImg.astype(np.float32)
            curImg = curImg.astype(np.float32)
        results['filename'] = filename
        results['preImg'] = preImg
        results['curImg'] = curImg
        results['img_shape'] = preImg.shape
        results['ori_shape'] = preImg.shape
        return results
...
@PIPELINES.register_module
class LoadCdAnnotations(object):
