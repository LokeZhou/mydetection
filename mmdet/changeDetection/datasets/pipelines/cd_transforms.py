...
class CdResize(object):
    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            preImg, scale_factor = mmcv.imrescale(
                results['preImg'], results['scale'], return_scale=True)
            curImg, scale_factor = mmcv.imrescale(
                results['curImg'], results['scale'], return_scale=True)

        else:
            preImg, w_scale, h_scale = mmcv.imresize(
                results['preImg'], results['scale'], return_scale=True)
            curImg, w_scale, h_scale = mmcv.imresize(
                results['curImg'], results['scale'], return_scale=True)

            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
        results['preImg'] = preImg
        results['curImg'] = curImg
        results['img_shape'] = preImg.shape
        results['pad_shape'] = preImg.shape  # in case that there is no padding
...
class CdRandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            # flip image
            results['preImg'] = mmcv.imflip(results['preImg'])
            results['curImg'] = mmcv.imflip(results['curImg'])
            # flip bboxes
            for key in results.get('bbox_fields', []):
...
class CdPad(object):
    ...
    def _pad_img(self, results):
        if self.size is not None:
            pre_padded_img = mmcv.impad(results['preImg'], self.size)
            cur_padded_img = mmcv.impad(results['curImg'], self.size)

        elif self.size_divisor is not None:
            pre_padded_img = mmcv.impad_to_multiple(
                results['preImg'], self.size_divisor, pad_val=self.pad_val)
            cur_padded_img = mmcv.impad_to_multiple(
                results['curImg'], self.size_divisor, pad_val=self.pad_val)

        results['preImg'] = pre_padded_img
        results['curImg'] = cur_padded_img
        results['pad_shape'] = pre_padded_img.shape
...
class CdNormalize(object):
    ...
    def __call__(self, results):
        results['preImg'] = mmcv.imnormalize(results['preImg'], self.mean, self.std,
                                          self.to_rgb)
        results['curImg'] = mmcv.imnormalize(results['curImg'], self.mean, self.std,
                                             self.to_rgb)
